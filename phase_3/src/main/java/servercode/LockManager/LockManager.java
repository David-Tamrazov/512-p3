...
public class LockManager implements Serializable
{
    ...
    public boolean unlockTransactionLocks(int xid) {
	    return UnlockAll(xid);
    }
    
    public boolean Lock(int xid, String strData, int lockType) throws DeadlockException {
    
    	System.out.println("\nRequesting lock of type: " + lockType);
        ...
        try {
            ...
            while (bConflict) {
                synchronized (this.lockTable) {
                    ...
                    if (!bConflict) {
                        ...
                        if (bConvert.get(0) == true) {
                            ...
                            int oldLock = lockType == 1 ? 0 : 1;
                            
                            TrxnObj olt = new TrxnObj(xid, strData, oldLock);
							DataObj old = new DataObj(xid, strData, oldLock);
                            
                            // remove the old locks from the hashtable
                            this.lockTable.remove(olt);
                            this.lockTable.remove(old);
                            
                            System.out.println("\nLock table inbetween conversion: ");
                            printLockTable(trxnObj);      

							System.out.println("\nGetting lock of type : " + trxnObj.getLockType() + "\n");
                            // add the new lock they're requesting 
                            this.lockTable.add(dataObj);
                            this.lockTable.add(trxnObj);
                            
                            System.out.println("\nLock table after conversion: ");
                            printLockTable(trxnObj);


                        } else {
                        
                        	System.out.println("Lock table before request: ");
                        	printLockTable(trxnObj);
                        	
                        	
                        	System.out.println("Getting lock of type : " + trxnObj.getLockType() + "\n");
                        	
                            // a lock request that is not lock conversion
                            this.lockTable.add(trxnObj);
                            this.lockTable.add(dataObj);
                            
                        	System.out.println("Lock table after request: ");
                        	printLockTable(trxnObj);
                        }
                    }
                }
                if (bConflict) {
                    ...
                    System.out.println("Mad conflicted right now yo.");
                    System.out.println("System obj put on hold: " + dataObj.toString());
			...
			System.out.println("This lock threw us in a deadlock: " + dataObj.toString());
			System.out.println("This transaction got aborted: "+ deadlock.getXID());
            throw deadlock;
        }
    ...
    public boolean LockConflict(DataObj dataObj, BitSet bitset) throws DeadlockException, RedundantLockRequestException {
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {
                ...
                if (dataObj.getLockType() == DataObj.READ) {    
					...
					if (dataObj2.getLockType() == DataObj.WRITE) {
						...
						System.out.println("This shit redundant!");
        ...
        System.out.println("\nLock conflict returning false here.");
    ...
    private void printLockTable(TrxnObj t) {
		
		synchronized(this.lockTable) {
	                            
	    	Vector vect = this.lockTable.elements(t);
			...
			for (int i = (size - 1); i >= 0; i--) {
				...
				System.out.println(trxnObj.toString());
			}
